...
def add_user(user_dict, user_id):
    ...
    #if user hasn't ranked - fill as 0
    user_isin_films = titles["title"].isin(list(user_dict.values()))
    #isin return T/F for values passed to it --> if a row has passed value, it returns true. Returns entire db of T/F
...
def cosine_similarity(user_id, rankings_pd, k=5):
    ...
    output_list = list(output_dict.keys())[:k]
